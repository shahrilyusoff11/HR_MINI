        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('performance-reviews.update', $performanceReview) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        @for($i = 1; $i <= 5; $i++)
                                            <label class="flex items-center">
                                                <input type="radio" name="rating" value="{{ $i }}" 
                                                       {{ old('rating', $performanceReview->rating) == $i ? 'checked' : '' }} 
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="comments" name="comments" class="form-input" rows="4" required>{{ old('comments', $performanceReview->comments) }}</textarea>
                                <x-input-error class="mt-2" :messages="$errors->get('comments')" />
                            </div>

                            <div class="md:col-span-2">
                                <x-input-label for="goals" :value="__('Goals & Development Plan')" />
                                <textarea id="goals" name="goals" class="form-input" rows="4" required>{{ old('goals', $performanceReview->goals) }}</textarea>
